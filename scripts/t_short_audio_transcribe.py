...
def process_audio(speaker, wavfile, parent_dir, target_sr, lang2token, model, anno_lock, speaker_annos):
    try:
        # Assume original file is named like "0.wav", "1.wav", etc.
        ...
        if lang is None or text is None:
            print(f"Transcription failed for {save_path}\n")
            return None
        if lang not in list(lang2token.keys()):
            print(f"{lang} not supported, ignoring\n")
            return None
        anno = save_path + "|" + speaker + "|" + lang2token[lang] + text + lang2token[lang] + "\n"
        with anno_lock:
            speaker_annos.append(anno)
        return anno
    except Exception as e:
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for speaker in speaker_names:
            for wavfile in [f for f in list(os.walk(parent_dir + speaker))[0][2] if not f.startswith("processed_") and f.endswith(".wav")]:
                futures.append(executor.submit(process_audio, speaker, wavfile, parent_dir, target_sr, lang2token, model, anno_lock, speaker_annos))
